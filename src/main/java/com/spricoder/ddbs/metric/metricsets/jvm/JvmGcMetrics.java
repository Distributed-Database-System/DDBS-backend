...
public class JvmGcMetrics implements IMetricSet, AutoCloseable {
  ...
  public void bindTo(MetricService metricService) {
    ...
    metricService.createAutoGauge("jvm_gc_max_data_size_bytes", maxDataSize, AtomicLong::get);

    AtomicLong liveDataSize = new AtomicLong();
    metricService.createAutoGauge("jvm_gc_live_data_size_bytes", liveDataSize, AtomicLong::get);

    Counter allocatedBytes = metricService.getOrCreateCounter("jvm_gc_memory_allocated_bytes");

    Counter promotedBytes =
        (oldGenPoolName == null)
            ? null
            : metricService.getOrCreateCounter("jvm_gc_memory_promoted_bytes");
    ...
    for (GarbageCollectorMXBean mbean : ManagementFactory.getGarbageCollectorMXBeans()) {
      ...
      NotificationListener notificationListener =
          (notification, ref) -> {
            ...
            Timer timer =
                metricService.getOrCreateTimer(timerName, "action", gcAction, "cause", gcCause);
