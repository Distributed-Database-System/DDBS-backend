...
public class BlogController {
  ...
  public ResponseEntity<PageList<UserVO>> listUser(ListUserReq req) {
    return ResponseUtils.success(
        page(
            blogService.getUserList(req.getUid(), req.getName(), 1, 20),
            req.getPageNo(),
            req.getPageSize()));
  }
  ...
  public ResponseEntity<PageList<ArticleVO>> listArticle(ListArticleReq req) {
    return ResponseUtils.success(
        page(
            blogService.getArticleList(req.getAid(), req.getTitle(), 1, 20),
            req.getPageNo(),
            req.getPageSize()));
  }
  ...
  public ResponseEntity<byte[]> getImageAsResource() throws IOException {
    byte[] bytes =
        StreamUtils.copyToByteArray(servletContext.getResourceAsStream("/image/img.png"));
  ...
  public void queryPicture(@PathVariable String pictureName, HttpServletResponse response) {
    try {
      ...
      StreamUtils.copy(
          servletContext.getResourceAsStream("/image/img.png"), response.getOutputStream());
